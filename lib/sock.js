...
function InitSocketIO(server){
    ...
    const onConnection = (socket) =>{
        let referer = socket.request.headers.host;
        let domain = referer.split(":")[0];
    ...
    //save answer to file
    const saveAnswerInFile = (req) => {
        let filename = req.studentId+".json";
        let filepath = `${process.env.ANSWER_PATH}${filename}`;
        ...
        logDebug(`[{(ts:"${moment(new Date()).format("YYYY-MM-DD HH:mm:ss")}",hID: ${req.historyId}, edID:${req.examdetailsId}, qID:${req.questionId}, a:${req.answer===""?0:req.answer})}]`);
    }
    //send log to client
