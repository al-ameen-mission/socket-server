...
function InitSocketIO(server){
    ...
    const onConnection = (socket) =>{
        ...
        let session_id = socket.handshake.query.sessionId;
        let last_socket = connected_students[studentId];
        if (last_socket && last_socket.id!==socket.id) {
            if(last_socket.session_id===session_id) {
                console.log("Forcing to show duplicate tabe warning.");
                io.to(last_socket.id).emit("duplicate_tab", {});
            } else {
                console.log("Forcing to logout from another devices.");
                io.to(last_socket.id).emit("force_logout", {});
            }
            connected_students[socket.handshake.query.studentId] = {
                id:socket.id,
                session_id
            };
        }
