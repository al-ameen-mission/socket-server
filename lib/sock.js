...
function InitSocketIO(server){
    ...
    const onConnection = (socket) =>{
        ...
        let last_socket = connected_students[socket.handshake.query.studentId];
        if (last_socket && last_socket!==socket.id)
            io.to(last_socket).emit("force_logout", {});
        connected_students[socket.handshake.query.studentId] = socket.id;
